...
class TestBpmnUtil(unittest.TestCase):
    ...
    def test_raw_proc_to_digraph(self):
        ...
        self.assertEqual(len(digraph.keys()), 4)
        self.assertListEqual(list(digraph), ['StartEvent_One', 'Task_One', 'Task_Three', 'Task_Two'])
        self.assertListEqual(list(digraph.values()), [{'Task_One'}, {'Task_Two'}, {'EndEvent_One'}, {'Task_Three'}])
...
class TestBPMNComponent(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(out.namespace, self._wfprops.namespace)
        self.assertEqual(out.is_in_shared_ns, self._wfprops.namespace_shared)
        self.assertEqual(out.k8s_url, 'http://collect.default:5000/')
        self.assertEqual(out.envoy_host, 'collect.default.svc.cluster.local')
        self.assertEqual(out.annotation, {'preexisting': False, 'service': {'host': 'collect', 'port': 5000}})
        self.assertEqual(out.path, '/')
