...
cd ../.. && docker build -t flowd -f deploy/Dockerfile.flowd . && docker build -t healthd -f deploy/Dockerfile.healthd . && docker build -t catch-gateway:1.0.0 -f deploy/Dockerfile.catch .
