...
INCOMING_KAFKA_TOPIC = os.getenv('INCOMING_KAFKA_TOPIC', None)
OUTGOING_KAFKA_TOPIC = os.getenv('OUTGOING_KAFKA_TOPIC', None)
FORWARD_TASK_ID = os.getenv('FORWARD_TASK_ID', '')

FUNCTION = os.getenv('REXFLOW_RELIABLE_CONTROLLER_FUNCTION', 'START')
...
def send_to_stream(data, flow_id, wf_id, content_type):
    ...
    producer.produce(
        OUTGOING_KAFKA_TOPIC,
        data,
        headers=headers,
    )
    producer.poll(0)  # good practice: flush the toilet
