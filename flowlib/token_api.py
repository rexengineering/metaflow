...
class TokenPool:
    ...
    def is_done(self) -> bool:
        return self.size == self.lost + self.complete
    ...
    def __str__(self):
        return f'{self.name} siz:{self.size} avl:{self.avail} ' \
            f'act:{self.active} lst:{self.lost} ' \
            f'com:{self.complete} don:{self.is_done()}'
